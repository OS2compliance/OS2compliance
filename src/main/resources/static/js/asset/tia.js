document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById('setTiaOptOutCheckbox')
        .addEventListener('change', function () { assetDetailsService.setTiaOptOut(this); });

    document.getElementById('tiaOptOutText')
        .addEventListener('change', function () { assetDetailsService.updateTiaOptOutReason(this); });

    const editTIABtn = document.getElementById('editTIABtn');
    const cancelTIABtn = document.getElementById('cancelTIABtn');
    if (editTIABtn) editTIABtn.addEventListener('click', () => setTIAEditState(true));
    if (cancelTIABtn) cancelTIABtn.addEventListener('click', () => setTIAEditState(false));
    ...
    if (tiaLinkEditBtn) tiaLinkEditBtn.addEventListener('click', tiaLinkEditStart);
    if (tiaLinkSaveBtn) tiaLinkSaveBtn.addEventListener('click', tiaLinkSave);
    if (tiaLinkCancelBtn) tiaLinkCancelBtn.addEventListener('click', tiaLinkCancel);
    if (tiaLinkInput) tiaLinkInput.addEventListener('keydown', function (e) {
        if (e.key === 'Enter') {
            e.preventDefault();
            tiaLinkSave();
        }
