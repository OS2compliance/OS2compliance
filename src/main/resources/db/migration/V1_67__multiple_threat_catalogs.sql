...
-- Tilføj foreign keys separat
ALTER TABLE threat_assessment_catalogs
ADD CONSTRAINT fk_threat_assessment_catalogs_assessment
FOREIGN KEY (threat_assessment_id) REFERENCES threat_assessments(id) ON DELETE CASCADE;

ALTER TABLE threat_assessment_catalogs
ADD CONSTRAINT fk_threat_assessment_catalogs_catalog
FOREIGN KEY (threat_catalog_identifier) REFERENCES threat_catalogs(identifier) ON DELETE CASCADE;
